...
describe('trickle', () => {
  ...
  it('the trickle honours the share — the rule itself is share.test.ts', () => {
    const wood = sample().find((h3) => resourceOf(h3) === 'wood') as string;
    const [c] = cellsAt([wood]);
    const base = trickle([c as Cell], 10 * HOUR, T0).wood;
    const shared = { with: 'r', mineAtImport: 100, theirsAtImport: 100, myDays: 6, theirDays: 2 };
    expect(trickle([{ ...(c as Cell), shared }], 10 * HOUR, T0).wood).toBe(base * 0.75);
  });
});
