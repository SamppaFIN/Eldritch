...
describe('seededTerrainOf', () => {
  ...
  it('reads the Pyhäjärvi water as a lake', () => {
    expect(seededTerrainOf(at(61.465, 23.727))).toEqual({ kind: 'lake', source: 'seed' });
  });

  it('reads the statue block as a market — a shop and a bar are there', () => {
    expect(seededTerrainOf(at(61.4578, 23.728))).toEqual({ kind: 'market', source: 'seed' });
  });

  it('a shore cell reads coast, not the lake it sits beside — order matters', () => {
    // The marina shore: inside both the coast circle and the Pyhäjärvi circle. Coast is
    // listed first, so it wins.
    expect(seededTerrainOf(at(61.46, 23.7245))).toEqual({ kind: 'coast', source: 'seed' });
  });

  it('a cell inside the box matching no region is surveyed plain', () => {
    expect(seededTerrainOf(at(61.452, 23.716))).toEqual({ kind: 'plain', source: 'seed' });
  });
});

describe('the survey feeds the rest of the terrain rules', () => {
  it('terrainOf returns the surveyed value in the box', () => {
    expect(terrainOf(at(61.465, 23.727)).source).toBe('seed');
  });

  it('a lake cell yields food through resourceOf', () => {
    expect(resourceOf(at(61.465, 23.727))).toBe('food');
  });

  it('the survey beats a stored tile terrain on the same cell', () => {
    const h3 = at(61.4578, 23.728); // market by survey
    const cell: Cell = {
      h3,
      ownerId: 'me',
      strength: 100,
